...
function getAllBranchesBySearch($pdo, $search_query) {
    $sql = "SELECT * FROM branches WHERE 
            CONCAT(address, Manager, contact_number, date_added, added_by, last_updated, last_updated_by) 
            LIKE ?";
...
function insertAnActivityLog($pdo, $operation, $branch_id, $address, 
        $Manager, $contact_number, $username) {

    $sql = "INSERT INTO activity_logs (operation, branch_id, address, 
            Manager, contact_number, username) VALUES(?,?,?,?,?,?)";
...
function insertABranch($pdo, $address, $Manager, $contact_number, $added_by) {
    $response = array();
    $sql = "INSERT INTO branches (address, Manager, contact_number, added_by) VALUES(?,?,?,?)";
    $stmt = $pdo->prepare($sql);
    $insertBranch = $stmt->execute([$address, $Manager, $contact_number, $added_by]);
    ...
    if ($insertBranch) {
        ...
        $insertAnActivityLog = insertAnActivityLog($pdo, "INSERT", $getBranchID['branch_id'], 
            $getBranchID['address'], $getBranchID['Manager'], 
...
function updateBranch($pdo, $address, $Manager, $contact_number, 
    ...
    $stmt = $pdo->prepare($sql);
    $updateBranch = $stmt->execute([$address, $Manager, $contact_number, 
        $last_updated, $last_updated_by, $branch_id]);
...
function deleteABranch($pdo, $branch_id) {
    ...
    $insertAnActivityLog = insertAnActivityLog($pdo, "DELETE", $getBranchByID['branch_id'], 
        $getBranchByID['address'], $getBranchByID['Manager'], 
